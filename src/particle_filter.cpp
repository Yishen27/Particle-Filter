  ...
  for (int i = 0; i<particles.size(); i++){
    ...
    */
    for (int j = 0; j<observations.size();j++){
      ...
      L.y = y_p + (sin(theta_p) * observations[j].x) + (cos(theta_p) * observations[j].y);
    ...
    for (int m=0;m<transformed_ob.size();m++){
      ...
      for (int l = 0; l<LM_in_range.size();l++){
        if (transformed_ob[m].id == LM_in_range[l].id){
          mu_x = LM_in_range[l].x;
          mu_y = LM_in_range[l].y;
...
void ParticleFilter::resample() {
  ...
  */
  ...
  std::mt19937 gen(rd());
  std::uniform_int_distribution<> dis_in(0, N-1);
  std::uniform_real_distribution<> dis(0, 1);
  int index = dis_in(gen);
